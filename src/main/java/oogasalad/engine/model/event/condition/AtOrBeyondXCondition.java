...
/**
 * A {@link Condition} that is satisfied when a {@link GameObject}'s X-coordinate
 * is greater than or equal to a specified threshold.
 * <p>
 * The threshold value is provided in the {@code doubleParams} map under the key
 * {@code "x"}.
 * </p>
 */
public class AtOrBeyondXCondition implements Condition {
  ...
  /**
   * Determines whether the given {@code gameObject} has reached or passed
   * the X position specified by the parameter {@code "x"}.
   *
   ...
   * @param doubleParams a map containing numeric parameters;
   *                     must include the key {@code "x"} whose value is the
   *                     target X-coordinate threshold
   * @return {@code true} if {@code gameObject.getXPosition()} is greater than
