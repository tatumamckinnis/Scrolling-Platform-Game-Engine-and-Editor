...
public class EnemyFactory implements GameObjectFactory {
  ...
  public GameObject createGameObject(UUID uuid, int blueprintID, int hitBoxX, int hitBoxY,
      int hitBoxWidth, int hitBoxHeight, int layer, String name, String group, SpriteData spriteData,
      DynamicVariableCollection params, List<Event> events) {
    return new Enemy(uuid, blueprintID, hitBoxX, hitBoxY, hitBoxWidth, hitBoxHeight, layer, name, group, spriteData, params, events);
  }
}
