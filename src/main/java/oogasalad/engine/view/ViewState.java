...
/**
 * Holds the internal state needed to manage the current view of the application,
 * such as stage, current display, input keys, and game manager reference. Allows for toggling
 * between screens/states by external classes such as the factory package.
 *
 * Acts as a central, mutable state object that can be shared across internal view components.
 * Is only accessible by the factory package to encapsulate the View's state from external view packages.
 */
public class ViewState {
  ...
  private DefaultView myDefaultView;
  ...
  private static void checkClassCaller() {
    ...
    for (StackTraceElement element : stackTrace) {
      if (element.getClassName().equals(ALLOWED_CLASS_NAME_1) || element.getClassName().equals(ALLOWED_CLASS_NAME_2)) {
        return;
      }
    }
    LOG.warn("Class does not have access to variable bridge.");
    throw new SecurityException("Access denied: Only " + ALLOWED_CLASS_NAME_1 + " may call this method.");
  }
}
