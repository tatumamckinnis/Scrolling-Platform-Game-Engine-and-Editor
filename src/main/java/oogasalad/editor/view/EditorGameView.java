...
public class EditorGameView extends Pane implements EditorViewListener {
   ...
   *                                  non-positive.
   */
  public EditorGameView(int width, int height, int cellSize, EditorController editorController) {
    if (editorController == null) {
      throw new IllegalArgumentException("EditorController cannot be null.");
    }
    if (width <= 0 || height <= 0 || cellSize <= 0) {
      LOG.error("Invalid dimensions provided: width={}, height={}, cellSize={}", width, height,
          cellSize);
    ...
    this.gridCanvas = new Canvas(width, height);
    this.objectCanvas = new Canvas(width, height);
    ...
    initializeView();
    LOG.info("EditorGameView initialized with size {}x{}, cell size {}", width, height, cellSize);
  }
  ...
  /**
   * Draws the background grid lines, including the horizon line, on the dedicated grid canvas.
   */
  public void drawGrid() {
    gridGraphicsContext.setFill(GRID_BACKGROUND_COLOR);
    gridGraphicsContext.fillRect(0, 0, viewWidth, viewHeight);
    gridGraphicsContext.setStroke(GRID_LINE_COLOR);
    gridGraphicsContext.setLineWidth(GRID_LINE_WIDTH);
    for (int x = 0; x <= viewWidth; x += cellSize) {
      gridGraphicsContext.strokeLine(x, 0, x, viewHeight);
    }
    for (int y = 0; y <= viewHeight; y += cellSize) {
      gridGraphicsContext.strokeLine(0, y, viewWidth, y);
    }
    int horizonY = (int) (viewHeight * GRID_HORIZON_RATIO);
    gridGraphicsContext.setStroke(GRID_HORIZON_COLOR);
    gridGraphicsContext.setLineWidth(GRID_HORIZON_WIDTH);
    gridGraphicsContext.strokeLine(0, horizonY, viewWidth, horizonY);
  ...
  private void redrawObjectsInternal() {
    objectGraphicsContext.clearRect(0, 0, viewWidth, viewHeight);
