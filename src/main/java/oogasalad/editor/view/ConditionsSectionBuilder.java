 ...
 * Handles condition groups, adding/removing conditions by type (String) and index,
 * and provides UI for parameter editing. Uses separate functional interfaces for handlers
 * and a separate class for display items.
 */
public class ConditionsSectionBuilder {
   ...
   * @param conditionTypeSupplier Supplier providing a List of available condition type names (Strings).
   * @param addGroupHandler       Runnable executed when the "Add Group" button is clicked.
   * @param removeGroupHandler    IntConsumer executed when "Remove Group" is clicked, accepting the group index.
   * @param addConditionHandler   Handler (implementing {@link AddConditionHandler}) executed when "Add Condition" is clicked.
   * @param removeConditionHandler Handler (implementing {@link RemoveConditionHandler}) executed when "Remove Condition" is clicked.
   * @param editConditionParamHandler Handler (implementing {@link EditConditionParamHandler}) executed when a parameter value is modified.
  ...
  /**
   * Builds and returns the complete UI Node for the conditions management section.
   * This includes controls for managing condition groups, selecting and adding/removing conditions,
   * displaying the list of conditions, and an area for editing parameters of the selected condition.
   *
  ...
  /**
   * Creates the horizontal layout (HBox) containing buttons for adding and removing condition groups.
   *
   * @return The {@code HBox} node with group management buttons ("Add Group", "Remove Selected Group").
   */
  private HBox createGroupButtonRow() {
    ...
    Button removeGroupButton = createButton(KEY_REMOVE_GROUP_BUTTON, e -> handleRemoveGroupAction());
  ...
  /**
   * Creates the horizontal layout (HBox) containing the condition type ComboBox and the buttons
   * for adding and removing individual conditions.
   *
  ...
  private HBox createConditionSelectionRow() {
    ...
    Button addConditionButton = createButton(KEY_ADD_CONDITION_BUTTON, e -> handleAddConditionAction());
    addConditionButton.setId("addConditionButton");
    addConditionButton.setMaxWidth(Double.MAX_VALUE);

    Button removeConditionButton = createButton(KEY_REMOVE_CONDITION_BUTTON, e -> handleRemoveConditionAction());
  ...
  /**
   * Initializes and configures the ComboBox used for selecting condition types.
   * Populates it with values obtained from the {@code conditionTypeSupplier}.
   */
  private void setupConditionTypeComboBox() {
    conditionTypeComboBox = new ComboBox<>(FXCollections.observableArrayList(conditionTypeSupplier.get()));
  ...
  /**
   * Initializes and configures the ListView that displays the conditions.
   * Sets the cell factory to display {@link ConditionDisplayItem} objects and adds a listener
   * to update the parameters pane whenever the selection changes.
   */
  private void setupConditionsListView() {
    ...
    conditionsListView.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
      updateParametersPane(newVal);
    });
  }

  /**
   * Creates the UI section dedicated to displaying and editing parameters of the currently selected condition.
   * This section includes a header and a scrollable pane containing the parameter editors.
   *
   * @return A {@code Node} (specifically a VBox) acting as the container for the parameter editing UI.
   */
  ...
  /**
   * Handles the action triggered by the "Add Group" button by invoking the {@code addGroupHandler}.
   */
   ...
   * If a condition is selected in the ListView, it invokes the {@code removeGroupHandler}
   * with the group index of the selected condition. Otherwise, logs a warning.
   */
   ...
   * Retrieves the selected condition type from the ComboBox. Determines the target group index
   * (either from the selected item in the ListView or defaulting to 0). Invokes the
   * {@code addConditionHandler} with the target group index and selected type.
   */
   ...
   * If a condition is selected in the ListView, it invokes the {@code removeConditionHandler}
   * with the group and condition indices of the selected item. Otherwise, logs a warning.
   */
   ...
   * Clears the pane if no condition is selected. Otherwise, dynamically creates Label-TextField pairs
   * for each String and Double parameter found in the selected condition's {@link ExecutorData}.
   * Attaches listeners to the TextFields to invoke the {@code editConditionParamHandler} upon value change.
   *
   * @param selectedItem The currently selected {@link ConditionDisplayItem}, or {@code null} if none is selected.
   */
  private void updateParametersPane(ConditionDisplayItem selectedItem) {
    ...
    if (data.getStringParams() != null) {
      for (Map.Entry<String, String> entry : data.getStringParams().entrySet()) {
        ...
        valueField.textProperty().addListener((obs, oldVal, newVal) -> {
          editConditionParamHandler.handle(selectedItem.groupIndex, selectedItem.conditionIndex, entry.getKey(), newVal);
        });
    ...
    if (data.getDoubleParams() != null) {
      for (Map.Entry<String, Double> entry : data.getDoubleParams().entrySet()) {
        ...
        valueField.textProperty().addListener((obs, oldVal, newVal) -> {
          try {
            Double doubleVal = Double.parseDouble(newVal);
            editConditionParamHandler.handle(selectedItem.groupIndex, selectedItem.conditionIndex, entry.getKey(), doubleVal);
          } catch (NumberFormatException e) {
            LOG.warn("Invalid double format for param '{}': {}", entry.getKey(), newVal);
          }
        });
        grid.add(nameLabel, 0, rowIndex);
        grid.add(valueField, 1, rowIndex++);
  ...
  /**
   * Updates the conditions ListView to display the provided condition groups.
   * Converts the nested list structure (List of groups, each containing a List of ExecutorData)
   * into a flat ObservableList of {@link ConditionDisplayItem} suitable for the ListView.
   * Clears the parameter pane after updating the list, as the selection might become invalid.
   *
   * @param conditionGroups A {@code List<List<ExecutorData>>} representing the condition groups and their conditions.
        ...
        if (group != null) {
          for (int j = 0; j < group.size(); j++) {
            ...
            if (conditionData != null) {
              displayItems.add(new ConditionDisplayItem(i, j, conditionData)); // Uses separate class
            }
          }
        }
      }
  ...
  /**
   * Creates a styled header label using text retrieved from the resource bundle based on the provided key.
   *
  ...
  /**
   * Creates a styled button with text from the resource bundle and assigns the provided action handler.
   *
  ...
  /**
   * Creates a horizontal layout (HBox) to hold one or more buttons, applying specified spacing
   * and aligning the buttons to the center-left.
   *
